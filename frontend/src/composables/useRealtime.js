      ...
      dc.addEventListener("message", async (e) => {
        ...
        if (isFunctionDone && parsed.name && parsed.arguments) {
          ...
          try {
            ...
            // Send function response back to model via data channel
            if (dcRef.value && json.status === 'ok') {
            // After executing the function, send a conversation.item.create event with the summary so the model/browser can speak it
            let summaryText;
            ...
            if (summaryText) {
              // Send summary back as a new conversation.item.create event
              const convEvent = {
